...
The analysis looks the trend in TMIN (the minimum daily temperature) in for Winter and Summer months. While changes are not visible in the overall trend, using a threshold level detection scheme, we can clearly see an upturn in TMIN during Winter months. The trend is not apparent dueing summer months. This suggests the effects of climate change are more subtle than one might expect, affecting minimum temperatures during colder months rather than maximum temperatures during hotter months. 

Full code is available on my [Github](http://github.com/ww44ss/MinTempTrends)
...
This plot looks at the number of days above 0C, the freezing point. This is significant biologically since many plants adn animals suffer more severe damage below freezing, so changes on flora andfauna might be expected for days with fewer days with zero degree days. 
...
The number of days with TMIN above zero appears to be increasing in the last few years. This is consistent with other pbservations that the effects of Climate Change are most prevalent after about 1980. 
...
The temperature data from this station don't on the surface appear to show signficant change. However, by looking at a thresold level, we can see clearly the behavior of TMIN has changed for Winter months.  
