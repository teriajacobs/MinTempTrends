...
library(ggplot2)
        ...
        P1<-P1+stat_smooth()
        P1<-P1+xlab("Date")+ylab("Temperature (C)")+ggtitle("Min Temperatures vs. Date") + theme(axis.text.x = element_text(angle = 90, hjust = 1))+ coord_cartesian(ylim = c(-10, 10)) 
...
TempDataWinter<-TempData[TempData$MON == "Dec"|TempData$MON == "Jan"|TempData$MON == "Feb",]
...
TMINdata<-merge(TMINtotal, TMINsubcount)
...
head(TMINtotal)
       ...
       P1<-P1+stat_smooth()
       P1<-P1+xlab("YEAR")+ylab("Number of Days")+ggtitle("Days Below Median vs. Date") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
